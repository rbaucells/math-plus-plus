#ifndef MATHPP_IMPLEMENTATION_MATRIX_SPARSE_CSR_MATRIX_H
#define MATHPP_IMPLEMENTATION_MATRIX_SPARSE_CSR_MATRIX_H
...
struct CSRSparseMatrix {
     ...
     */
    CSRSparseMatrix(const std::size_t rows, const std::size_t columns) : rows_(rows), columns_(columns) {
     ...
     */
    CSRSparseMatrix(const std::size_t rows, const std::size_t columns, std::initializer_list<std::tuple<T, int, int> > initializerList) : rows_(rows), columns_(columns) {
        if (rows < 0 || columns < 0) {
     ...
     */
    CSRSparseMatrix(const CSRSparseMatrix<T>& other) : rows_(other.rows_), columns_(other.columns_) {
    ...
    */
    ...
    CSRSparseMatrix(const CSRSparseMatrix<OTHER_T>& other) : rows_(other.rows()), columns_(other.columns()) {
        colOffsets_ = new std::size_t[columns_ + 1];
    ...
    CSRSparseMatrix(const U& other) : rows_(other.rows()), columns_(other.columns()) {
        colOffsets_ = new std::size_t[columns_ + 1];
        ...
        for (std::size_t c = 0; c < columns_; c++) {
            for (std::size_t r = 0; r < rows_; r++) {
                CSRSparseMatrix<T>::set(c, r, other.get(c, r));
            }
        }
    }
     ...
     */
    CSRSparseMatrix(CSRSparseMatrix&& other) noexcept : rows_(other.rows_), columns_(other.columns_), nnz_(other.nnz_), values_(other.values_), colOffsets_(other.colOffsets_), rowIndices_(other.rowIndices_) {
     ...
     */
    CSRSparseMatrix<T>& operator=(const CSRSparseMatrix<T>& other) {
        if (this != &other) {
     ...
     */
    template<scalar OTHER_T> requires std::is_convertible_v<OTHER_T, T>
    CSRSparseMatrix<T>& operator=(const CSRSparseMatrix<OTHER_T>& other) {
    ...
    template<sparse_matrix_like U>
    CSRSparseMatrix<T>& operator=(const U& other) {
        if (columns_ != other.columns()) {
            delete[] colOffsets_;
            columns_ = other.columns();
            colOffsets_ = new std::size_t[columns_ + 1];
    ...
    * @note 'other' must be of same dimensions as this.
    */
    CSRSparseMatrix<T>& operator=(CSRSparseMatrix<T>&& other) noexcept {
        if (this != &other) {
            delete[] values_;
            delete[] rowIndices_;
            delete[] colOffsets_;
...
#endif // MATHPP_IMPLEMENTATION_MATRIX_SPARSE_CSR_MATRIX_H
