#ifndef MATHPP_IMPLEMENTATION_MATRIX_SPARSE_COMMON_ASSERTS_H
#define MATHPP_IMPLEMENTATION_MATRIX_SPARSE_COMMON_ASSERTS_H
...
inline void assert_same_dimensions(const T& a, const U& b, const ARGS&... args) {
    if (!(a.columns() == b.columns() && a.rows() == b.rows() && ((a.columns() == args.columns() && a.rows() == args.rows()) && ...))) {
        throw InvalidSizeException("Sparse matrices must all have same dimensions");
    }
}
...
inline void assert_square(const T& m) {
    if (m.columns() != m.rows()) {
        throw InvalidSizeException("Sparse matrix must be square");
    }
}

#endif // MATHPP_IMPLEMENTATION_MATRIX_SPARSE_COMMON_ASSERTS_H
