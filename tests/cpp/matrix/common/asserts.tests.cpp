...
TEST(assert_assert_square, given_wide_should_throw) {
    ...
    // act / assert
    ASSERT_THROW(assert_square(m), InvalidSizeException);
}
...
TEST(assert_assert_wide, given_square_throw) {
    ...
    // act / assert
    ASSERT_THROW(assert_wide(m), InvalidSizeException);
}
...
TEST(assert_assert_tall, given_square_throw) {
    ...
    // act / assert
    ASSERT_THROW(assert_tall(m), InvalidSizeException);
}
...
TEST(assert_can_multiple, given_non_multipliable_should_throw) {
    ...
    // act / assert
    ASSERT_THROW(assert_can_multiply(a, b), InvalidSizeException);
}
...
TEST(assert_same_dimensions, given_matrix_like_of_diff_dimensions_should_throw) {
    ...
    // act / assert
    ASSERT_THROW(assert_same_dimensions(a, b, c), InvalidSizeException);
}
