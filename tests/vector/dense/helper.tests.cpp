...
TEST(compare, given_f_dense_vector_and_f_dense_vector_should_return_true) {
    // arrange
    const DenseVector<float> a = {1, 2, 3};
    const DenseVector<float> b = {1, 2, 3};
    // act / assert
    ASSERT_TRUE((compare<DenseVector<float>, DenseVector<float>>(a, b, 0.001f)));
}

TEST(compare, given_f_dense_vector_and_d_dense_vector_should_return_true) {
    // arrange
    const DenseVector<float> a = {1, 2, 3};
    const DenseVector<double> b = {1, 2, 3};
    // act / assert
    ASSERT_TRUE((compare<DenseVector<float>, DenseVector<double>>(a, b, 0.001f)));
}

TEST(compare, given_f_dense_vector_and_i_dense_vector_should_return_true) {
    // arrange
    const DenseVector<float> a = {1, 2, 3};
    const DenseVector<int> b = {1, 2, 3};
    // act / assert
    ASSERT_TRUE((compare<DenseVector<float>, DenseVector<int>>(a, b, 0.001f)));
}

TEST(compare, given_f_dense_vector_and_cf_dense_vector_should_return_true) {
    // arrange
    const DenseVector<float> a = {1, 2, 3};
    const DenseVector<std::complex<float>> b = {{1, 0}, {2, 0}, {3, 0}};
    // act / assert
    ASSERT_TRUE((compare<DenseVector<float>, DenseVector<std::complex<float>>>(a, b, 0.001f)));
}

TEST(compare, given_cf_dense_vector_and_cf_dense_vector_should_return_true) {
    // arrange
    const DenseVector<std::complex<float>> a = {{1, 2}, {3, 4}, {5, 6}};
    const DenseVector<std::complex<float>> b = {{1, 2}, {3, 4}, {5, 6}};
    // act / assert
    ASSERT_TRUE((compare<DenseVector<std::complex<float>>, DenseVector<std::complex<float>>>(a, b, 0.001f)));
}

TEST(compare, given_f_dense_vector_and_f_dense_vector_should_return_false) {
    // arrange
    const DenseVector<float> a = {1, 2, 4};
    const DenseVector<float> b = {1, 2, 3};
    // act / assert
    ASSERT_FALSE((compare<DenseVector<float>, DenseVector<float>>(a, b, 0.001f)));
}

TEST(compare, given_f_dense_vector_and_d_dense_vector_should_return_false) {
    // arrange
    const DenseVector<float> a = {1, 3, 3};
    const DenseVector<double> b = {1, 2, 3};
    // act / assert
    ASSERT_FALSE((compare<DenseVector<float>, DenseVector<double>>(a, b, 0.001f)));
}

TEST(compare, given_f_dense_vector_and_i_dense_vector_should_return_false) {
    // arrange
    const DenseVector<float> a = {1.1f, 2, 3};
    const DenseVector<int> b = {1, 2, 3};
    // act / assert
    ASSERT_FALSE((compare<DenseVector<float>, DenseVector<int>>(a, b, 0.001f)));
}

TEST(compare, given_f_dense_vector_and_cf_dense_vector_should_return_false_1) {
    // arrange
    const DenseVector<float> a = {1, 2, 3};
    const DenseVector<std::complex<float>> b = {{1, 1}, {2, 0}, {3, 0}};
    // act / assert
    ASSERT_FALSE((compare<DenseVector<float>, DenseVector<std::complex<float>>>(a, b, 0.001f)));
}

TEST(compare, given_f_dense_vector_and_cf_dense_vector_should_return_false_2) {
    // arrange
    const DenseVector<float> a = {1, 2, 3};
    const DenseVector<std::complex<float>> b = {{2, 0}, {2, 0}, {3, 0}};
    // act / assert
    ASSERT_FALSE((compare<DenseVector<float>, DenseVector<std::complex<float>>>(a, b, 0.001f)));
}

TEST(compare, given_cf_dense_vector_and_cf_dense_vector_should_return_false_1) {
    // arrange
    const DenseVector<std::complex<float>> a = {{1, 3}, {3, 4}, {5, 6}};
    const DenseVector<std::complex<float>> b = {{1, 2}, {3, 4}, {5, 6}};
    // act / assert
    ASSERT_FALSE((compare<DenseVector<std::complex<float>>, DenseVector<std::complex<float>>>(a, b, 0.001f)));
}

TEST(compare, given_cf_dense_vector_and_cf_dense_vector_should_return_false_2) {
    // arrange
    const DenseVector<std::complex<float>> a = {{0, 2}, {3, 4}, {5, 6}};
    const DenseVector<std::complex<float>> b = {{1, 2}, {3, 4}, {5, 6}};
    // act / assert
    ASSERT_FALSE((compare<DenseVector<std::complex<float>>, DenseVector<std::complex<float>>>(a, b, 0.001f)));
}
