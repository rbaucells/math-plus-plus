...
def test_get_sequence_info():
    # arrange
    a = [25, -25, 25]
    aExpected = (numpy.dtypes.Int16DType(), mathpy.EType.scalar, 3)
    # act
    aResult = mathpy.get_sequence_info(a)
    # assert
    assert aResult == aExpected
    # arrange
    b = [3, -3, 2.6, 4]
    bExpected = (numpy.dtypes.Float64DType(), mathpy.EType.scalar, 4)
    # act
    bResult = mathpy.get_sequence_info(b)
    # assert
    assert bResult == bExpected
    # arrange
    c = [mathpy.DenseMatrix(numpy.dtypes.Int32DType(), 3, 3), mathpy.DenseMatrix(numpy.dtypes.UInt32DType(), 4, 2)]
    cExpected = (numpy.dtypes.Int64DType(), mathpy.EType.dense_matrix_like, 2)
    # act
    cResult = mathpy.get_sequence_info(c)
    # assert
    ...
    with pytest.raises(TypeError):
        mathpy.get_sequence_info(d)
    # arrange
    e = [2, 4 + 3j]
    eExpected = (numpy.dtypes.Complex128DType(), mathpy.EType.scalar, 2)
    # act
    eResult = mathpy.get_sequence_info(e)
    # assert
    assert eResult == eExpected
    # arrange
    f = [3, mathpy.DenseMatrix(numpy.dtypes.UInt8DType())]
    fExpected = (numpy.dtypes.UInt8DType(), mathpy.EType.invalid, 2)
    # act
    fResult = mathpy.get_sequence_info(f)
    # assert
...
def test_get_sequence_info_2d():
    # arrange
    a = [[4, -6], [10, 130], [4, 0]]
    aExpected = (numpy.dtypes.Int16DType(), mathpy.EType.scalar, 3, 2)
    # act
    aResult = mathpy.get_sequence_info_2d(a)
    # assert
    assert aResult == aExpected
    # arrange
    ...
    bExpected = (numpy.dtypes.Float64DType(), mathpy.EType.dense_matrix_like, 2, 2)
    # act
    bResult = mathpy.get_sequence_info_2d(b)
    # assert
    assert bResult == bExpected
    # arrange
    c = [[1, 2], [3, 4, 5]]
    # act / assert
    with pytest.raises(TypeError):
        mathpy.get_sequence_info_2d(c)
    # arrange
    d = [44, -6, 2 + 3j]
    # act / assert
    with pytest.raises(TypeError):
        mathpy.get_sequence_info_2d(d)
    # arrange
    e = [[1, 2], [[4, 5], [4, 1]]]
    # act / assert
    with pytest.raises(TypeError):
        mathpy.get_sequence_info_2d(e)

def test_get_array_info():
    # dt is object - mixed scalar types
    a = numpy.array([25, -25, 3.5], dtype=object)
    aExpected = (numpy.dtypes.Float64DType(), mathpy.EType.scalar, 3)
    # act
    aResult = mathpy.get_array_info(a)
    # assert
    assert aResult == aExpected
    ...
    bExpected = (numpy.dtypes.Int64DType(), mathpy.EType.dense_matrix_like, 2)
    # act
    bResult = mathpy.get_array_info(b)
    # assert
    assert bResult == bExpected
    # dt is defined - int32 array
    c = numpy.array([25, -25, 25], dtype=numpy.int32)
    cExpected = (numpy.dtypes.Int32DType(), mathpy.EType.scalar, 3)
    # act
    cResult = mathpy.get_array_info(c)
    # assert
    assert cResult == cExpected
    # mixed scalars and matrices - invalid et
    d = numpy.array([2, mathpy.DenseMatrix(numpy.dtypes.UInt8DType())], dtype=object)
    dExpected = (numpy.dtypes.UInt8DType(), mathpy.EType.invalid, 2)
    # act
    dResult = mathpy.get_array_info(d)
    # assert
...
def test_get_array_info_2d():
    ...
    aExpected = (numpy.dtypes.Int16DType(), mathpy.EType.scalar, 3, 2)
    # act
    aResult = mathpy.get_array_info_2d(a)
    # assert
    assert aResult == aExpected
    ...
    bExpected = (numpy.dtypes.Float64DType(), mathpy.EType.dense_matrix_like, 2, 2)
    # act
    bResult = mathpy.get_array_info_2d(b)
    # assert
    assert bResult == bExpected
    # dt is defined - 2D int32 array
    c = numpy.array([[1, 2], [3, 4]], dtype=numpy.int32)
    cExpected = (numpy.dtypes.Int32DType(), mathpy.EType.scalar, 2, 2)
    # act
    cResult = mathpy.get_array_info_2d(c)
    # assert
    assert cResult == cExpected
    # mixed scalars and matrices - invalid et
    d = numpy.array([[2, mathpy.DenseMatrix(numpy.dtypes.UInt8DType())], [3, 4]], dtype=object)
    dExpected = (numpy.dtypes.UInt8DType(), mathpy.EType.invalid, 2, 2)
    # act
    dResult = mathpy.get_array_info_2d(d)
    # assert
    assert dResult == dExpected
    # non-2d array raises
    e = numpy.array([1, 2, 3])
