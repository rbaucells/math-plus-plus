...
TEST(dense_matrix_base, given_sparse_matrix_should_return_false) {
    static_assert(!dense_matrix_like<CSCSparseMatrix<float>>, "dense_matrix_like is wrong, 'CSCSparseMatrix<float>' should not be dense_matrix_like");
}
#pragma endregion
...
TEST(custom_dense_matrix, given_dense_matrix_view_should_return_false) {
    static_assert(!custom_dense_matrix<DenseMatrixView<DenseMatrix<float>>>, "custom_dense_matrix is wrong, 'DenseMatrixView<DenseMatrix<float>>' should not be custom_dense_matrix");
}
#pragma endregion
...
TEST(assert_square, given_square_f_dense_matrix_should_return_true) {
    // arrange
    const DenseMatrix<float> a = {{1, 2, 3}, {4, 5, 6}, {7, 8, 9}};
    // act / assert
...
TEST(assert_square, given_non_square_f_dense_matrixshould_return_false) {
    // arrange
...
#pragma endregion
#pragma region assert_can_multiply
TEST(assert_can_multiply, given_multipliable_f_dense_matrices_should_return_true) {
    // assert
    const DenseMatrix<float> a(2, 3, false);
    const DenseMatrix<float> b(3, 4, false);
    const DenseMatrix<float> c(4, 5, false);
    // act
    ASSERT_NO_THROW(assert_can_multiply(a, b, c));
}

TEST(assert_can_multiply, given_non_multipliable_f_dense_matrices_should_return_true) {
    // assert
    const DenseMatrix<float> a(2, 3, false);
    const DenseMatrix<float> b(4, 4, false);
    const DenseMatrix<float> c(4, 5, false);
    // act
    ASSERT_THROW(assert_can_multiply(a, b, c), InvalidDimensionException);
}
#pragma endregion
