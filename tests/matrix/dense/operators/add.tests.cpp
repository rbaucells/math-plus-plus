...
//     // act
//     const DenseMatrix<float>& sum = run_with_budget<DenseMatrix<float>>({}, [&]() { return a + b + c; });
//     // assert
...
//     // act
//     const DenseMatrix<std::complex<float>>& sum = run_with_budget<DenseMatrix<std::complex<float>>>({}, [&]() { return a + b + c; });
//     // assert
//     ASSERT_TRUE(compare(Precision(0.001f), sum, expected));
// }
...
TEST(dense_matrix_add, given_f_dense_matrices_should_return_sum) {
    ...
    const DenseMatrix<float> expected = {{-2, 8, 18}, {-17, 68, 39}, {24, 14, 23}};
    // act
    const DenseMatrix<float> sum = add(a, b, c);
    // assert
    ASSERT_TRUE(compare(Precision(0.001f), sum, expected));
}

TEST(dense_matrix_add, given_cf_dense_matrices_should_return_sum) {
    // arrange
    const DenseMatrix<std::complex<float>> a = {{{2, 6}, {3, 6}, {0, 8}}, {{2, 1}, {3, 7}, {1, 0}}, {{4, 7}, {3, 3}, {5, 9}}};
    const DenseMatrix<std::complex<float>> b = {{{3, 67}, {2, 4}, {7, 7}}, {{9, 12}, {3, 64}, {3, 5}}, {{2, 2}, {0, 0}, {9, 2}}};
    const DenseMatrix<std::complex<float>> c = {{{3, 6}, {3, 3}, {1, 6}}, {{8, 4}, {4, 1}, {5, 1}}, {{0, 7}, {8, 5}, {3, 21}}};
    const DenseMatrix<std::complex<float>> expected = {{{8, 79}, {8, 13}, {8, 21}}, {{19, 17}, {10, 72}, {9, 6}}, {{6, 16}, {11, 8}, {17, 32}}};
    // act
    const DenseMatrix<std::complex<float>> sum = add(a, b, c);
    // assert
    ASSERT_TRUE(compare(Precision(0.001f), sum, expected));
}

TEST(dense_matrix_add, given_f_and_cf_dense_matrices_should_return_sum) {
    // arrange
    const DenseMatrix<std::complex<float>> a = {{{2, 6}, {3, 6}, {0, 8}}, {{2, 1}, {3, 7}, {1, 0}}, {{4, 7}, {3, 3}, {5, 9}}};
    const DenseMatrix<float> b = {{2, 5, 8}, {1, 7, 31}, {8, 2, 7}};
    const DenseMatrix<std::complex<float>> c = {{{3, 6}, {3, 3}, {1, 6}}, {{8, 4}, {4, 1}, {5, 1}}, {{0, 7}, {8, 5}, {3, 21}}};
