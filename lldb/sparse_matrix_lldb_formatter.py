...
def sparse_matrix_summary(valobj: lldb.SBValue, internal_dict):
    # get non-synthetic value to access actual SparseMatrix members
    ...
    if rows_int > _MAX_VIEW_DIM or columns_int > _MAX_VIEW_DIM:
        return f"SparseMatrix<{rows_int}x{columns_int}> nnz={nnz_int}"
...
class SparseMatrixSyntheticChildrenProvider:
    ...
    def _refresh(self):
        ...
        # nnz
        self.nnz: lldb.SBValue = self.valobj.GetChildMemberWithName("nnz_")
        self.nnz_int: int = self.nnz.GetValueAsUnsigned()
        # colOffsets, rowIndices, values
        self.col_offsets: lldb.SBValue = self.valobj.GetChildMemberWithName("colOffsets_")
        self.row_indices: lldb.SBValue = self.valobj.GetChildMemberWithName("rowIndices_")
        self.values: lldb.SBValue = self.valobj.GetChildMemberWithName("values_")
        # element types for array view children
        self.col_offsets_element_type: lldb.SBType = self.col_offsets.GetType().GetPointeeType()
        self.row_indices_element_type: lldb.SBType = self.row_indices.GetType().GetPointeeType()
        self.values_element_type: lldb.SBType = self.values.GetType().GetPointeeType()
    ...
    def get_child_at_index(self, index: int):
        ...
        if index == 2:
            return self.nnz
        if index == 3:
            return self.valobj.CreateValueFromAddress("colOffsets_", self.col_offsets.GetValueAsUnsigned(), self.col_offsets_array_type)
        if index == 4:
            return self.valobj.CreateValueFromAddress("rowIndices_", self.row_indices.GetValueAsUnsigned(), self.row_indices_array_type)
        if index == 5:
            return self.valobj.CreateValueFromAddress("values_", self.values.GetValueAsUnsigned(), self.values_array_type)
        # exit
        return None
