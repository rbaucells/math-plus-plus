...
struct Precision {
    ...
    Precision(T value) : value(value) {}

    Precision() : value(epsilon<T>()) {}
};
...
[[nodiscard]] bool compare(const T a, const OTHERS... others, const Precision<std::common_type_t<underlying_type_t<T>, underlying_type_t<OTHERS>...>> precision = {}) {
