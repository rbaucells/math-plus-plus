...
struct CSRCustomSparseMatrix {
    ...
    CSRCustomSparseMatrix() = delete;

    CSRCustomSparseMatrix(const CSRCustomSparseMatrix<T>& other) = delete;

    CSRCustomSparseMatrix(CSRCustomSparseMatrix<T>&& other) noexcept = delete;

    CSRCustomSparseMatrix<T>& operator=(const CSRCustomSparseMatrix<T>& other) = delete;

    CSRCustomSparseMatrix<T>& operator=(CSRCustomSparseMatrix<T>&& other) noexcept = delete;
    ...
    * @param rows Number of rows in matrix.
     ...
     */
    CSRCustomSparseMatrix(const std::size_t rows, const std::size_t columns, std::size_t*& colOffsets, std::size_t*& rowIndices, T*& values, std::size_t& nnz) : rows_(rows), columns_(columns), nnz_(nnz), values_(values), colOffsets_(colOffsets), rowIndices_(rowIndices) {
    }
