...
template<int ROWS, int COLUMNS, scalar T>
Matrix<ROWS, COLUMNS, T> Matrix<ROWS, COLUMNS, T>::rotationMatrixAboutOrigin(const T rot, const RotationType rotationType) requires (isSquare && COLUMNS == 2 && !isComplex) {
...
template<int ROWS, int COLUMNS, scalar T>
Matrix<ROWS + 1, COLUMNS + 1, T> Matrix<ROWS, COLUMNS, T>::rotationMatrixAboutPoint(const Vector<COLUMNS, T>& p, const T rot, const RotationType rotationType) requires (isSquare && COLUMNS == 2 && !isComplex) {
    T asRadians = convert(rotationType, RotationType::radians, rot);
...
template<int ROWS, int COLUMNS, scalar T>
Matrix<ROWS, COLUMNS, T> Matrix<ROWS, COLUMNS, T>::rotationMatrixAroundAxisThroughOrigin(const Vector<COLUMNS, T>& axis, const T rot, const RotationType rotationType) requires (isSquare && COLUMNS == 3 && !isComplex) {
    T asRadians = convert(rotationType, RotationType::radians, rot);
...
template<int ROWS, int COLUMNS, scalar T>
Matrix<ROWS + 1, COLUMNS + 1, T> Matrix<ROWS, COLUMNS, T>::rotationMatrixAroundAxisNotThroughOrigin(const Vector<COLUMNS, T>& axis, const Vector<COLUMNS, T>& p, const T rot, const RotationType rotationType) requires (isSquare && COLUMNS == 3 && !isComplex) {
...
// template<int ROWS, int COLUMNS, scalar T>
// Matrix<ROWS, COLUMNS, T> Matrix<ROWS, COLUMNS, T>::rotationMatrixInPlaneThroughOrigin(const Vector<COLUMNS, T>& v1, const Vector<COLUMNS, T>& v2, const T rot, const RotationType rotationType) requires (isSquare && COLUMNS >= 3 && !isComplex) {
//     T asRadians = convert(rotationType, RotationType::radians, rot);
//
//     T sin = std::sin(asRadians);
//     T cos = std::cos(asRadians);
...
//
// template<int ROWS, int COLUMNS, scalar T>
// Matrix<ROWS, COLUMNS, T> Matrix<ROWS, COLUMNS, T>::rotationMatrixInPLaneNotThroughOrigin(const Vector<COLUMNS, T>& v1, const Vector<COLUMNS, T>& v2, const Vector<COLUMNS, T>& p, const T rot, const RotationType rotationType) requires (isSquare && COLUMNS >= 3 && !isComplex) {
...
template<int ROWS, int COLUMNS, scalar T>
Matrix<ROWS, COLUMNS, T> Matrix<ROWS, COLUMNS, T>::reflectionMatrixAcrossPlaneThroughOrigin(const Vector<COLUMNS, T>& normal) requires (isSquare && !isComplex) {
    return identity() - 2 * (normal.outerProductMatrix(normal) / normal.euclidianNormSquared());
}

template<int ROWS, int COLUMNS, scalar T>
Matrix<ROWS + 1, COLUMNS + 1, T> Matrix<ROWS, COLUMNS, T>::reflectionMatrixAcrossPlaneNotThroughOrigin(const Vector<COLUMNS, T>& normal, const Vector<COLUMNS, T>& point) requires (isSquare && !isComplex) {
    Matrix<ROWS, COLUMNS, T> smallReflection = reflectionMatrixAcrossPlaneThroughOrigin(normal);

    Matrix<ROWS + 1, COLUMNS + 1, T> reflection = Matrix<ROWS + 1, COLUMNS + 1, T>::identity();

    for (int c = 0; c < COLUMNS; c++) {
        for (int r = 0; r < ROWS; r++) {
            reflection[c][r] = smallReflection[c][r];
        }
    }

    Matrix<ROWS + 1, COLUMNS + 1, T> translationToOrigin = Matrix<ROWS, COLUMNS, T>::translationMatrix(point);
    Matrix<ROWS + 1, COLUMNS + 1, T> translationBack = Matrix<ROWS, COLUMNS, T>::translationMatrix(-point);

    Matrix<ROWS + 1, COLUMNS + 1, T> result = translationToOrigin * reflection * translationBack;
